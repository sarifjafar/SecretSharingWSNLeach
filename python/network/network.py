...
class Network(list):
  ...
  def __init__(self, init_nodes=None):
    ...
    if init_nodes:
      ...
      logging.info('Base Station : {0}'.format(base_station.id))
      print('Base Station : {0}'.format(base_station.id))
  ...
  def simulate(self):
    ...
    for round_nb in range(0, cf.MAX_ROUNDS):
      ...
      logging.info("round %d: total remaining energy: %f" % print_args)
      ...
      # logging.info('Alive Node : {0} Energy remaining : {1}'.format(tracer['alive_nodes'], tracer['energies']))
  ...
  def _communication_phase(self):
    ...
    # logging.info('Rounds {0}: Alive Nodes {1}'.format(alive_nodes))

    if self.perform_two_level_comm == 1:
      logging.info('Performing two level comm')
      self._two_level_comm(alive_nodes)
    else:
      logging.info('Performing recursive comm')
  ...
  def _two_level_comm(self, alive_nodes):
    ...
    logging.info('******Sensor nodes transmitting data to CH******')
    for node in self.get_ordinary_nodes():
      node.transmit()
    logging.info('******CH transmitting data to BS******')
