...
import python.utils.shamir as sh
...
class Node(object):
  ...
  def _aggregate(self, msg_length):
    logging.info("node %d aggregating." % (self.id))
    ...
    logging.info('Energy consumed {0} and Total size of message sent {1}'.format(energy, self.amount_received))

  @_only_active_nodes
  def transmit(self, msg_length=None, destination=None):
    logging.info("node %d transmitting." % (self.id))
    # logging.info('{0} transmitting {1}'.format(self.id, self))
    if not msg_length:
      msg_length = self.tx_queue_size
    msg_length += cf.HEADER_LENGTH

    secret, share = sh.make_random_shares(3, 5)
    self._shares = share
    self._secret = secret

    logging.info('{0} : secret = {1} & shares = {2}'.format(self.id, self._secret, self._shares))
    ...
    logging.info("Node : {0} sending data to {1}".format(self.id, self._next_hop))
    ...
    destination.receive(msg_length, self._shares)
  ...
  @_only_active_nodes
  def receive(self, msg_length, shares):
    logging.info("node %d receiving." % (self.id))
    self._aggregate(msg_length - cf.HEADER_LENGTH)

    self.amount_received += msg_length
    secret = sh.recover_secret(shares)
  ...
  def sense(self):
    ...
    logging.info('{0} Data sense {1} & {2}'.format(self.id, self.tx_queue_size, self.amount_received))
    print("{0} Data sense {1} & {2}".format(self.id, self.tx_queue_size, self.amount_received))
