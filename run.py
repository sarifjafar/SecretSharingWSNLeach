...
def run_scenarios():
  ...
  for scenario in cf.scenarios:
    if type(scenario) is str:
      logging.info('Scenerio : {0}'.format(scenario))
      exec(scenario)
      continue
    ...
    # logging.info(routing_topology,optimization, aggregation, nickname)
    ...
    logging.info('Scenerio Name : {0}'.format(scenario_name))

    routing_protocol_class = eval(routing_topology)
    network.routing_protocol = routing_protocol_class()
    
    logging.info('Routing Class : {0}'.format(routing_protocol_class))
    ...
    logging.info('Aggregation Function : {0}'.format(aggregation_function))
